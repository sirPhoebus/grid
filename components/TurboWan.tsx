...
export const TurboWan: React.FC<TurboWanProps> = ({ initialData, onClearInitialData, onPreviewMedia }) => {
    ...
    const [workflowType, setWorkflowType] = useState<'turbowan' | 'qwen'>('turbowan');
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            for (let i = 0; i < total; i++) {
                ...
                const result = await ComfyUiService.runVideoWorkflow(workingAnchor, prompt, aspectRatio, workflowType);
    ...
    const getContainerStyles = (ar: string) => {
        ...
        if (ar === '16:9' || ar === '4:3') return { ...base, width: '100%', height: 'auto' };
        if (ar === '9:16' || ar === '3:4') return { ...base, width: 'auto', height: '100%' };
        return base;
    };
            ...
            <div className={`transition-all duration-500 ${isSidebarOpen ? 'pl-80 blur-[2px]' : 'pl-0'}`}>
                <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 py-4">
                    <div className="flex items-center justify-between">
                        ...
                        {stitchedVideoUrl && (
                            <button onClick={reset} className="px-6 py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-bold rounded-xl transition-all">New Project</button>
                        )}
                    </div>
                                ...
                                <div className="max-w-xl mx-auto py-12 space-y-8 animate-in fade-in duration-700">
                                    <div className="text-center space-y-4">
                                        <div className="relative inline-flex items-center justify-center">
                                            ...
                                            <div className="absolute inset-0 flex items-center justify-center"><span className="text-xl font-black text-indigo-400">{currentIteration}/{iterationCount}</span></div>
                                        </div>
                                    ...
                                    <div className="bg-slate-900/50 rounded-2xl p-6 border border-slate-800 backdrop-blur-md">
                                        ...
                                        <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                                            <div className="h-full bg-gradient-to-r from-indigo-600 to-purple-600 transition-all duration-500 ease-out" style={{ width: `${(currentIteration / iterationCount) * 100}%` }} />
                                        </div>
                                    </div>
                                </div>
                            )}
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-center gap-6 px-4">
                                    <div className={`flex-1 w-full max-w-[480px] transition-all duration-700 ${segments.length === 0 ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
                                        <div className="space-y-3">
                                            ...
                                            <div className={`bg-slate-900 rounded-2xl overflow-hidden border border-slate-800 shadow-2xl relative ${getAspectClass(aspectRatio)}`} style={getContainerStyles(aspectRatio)}>
                                                {segments.length > 0 && <video src={segments[segments.length - 1].videoUrl} className="w-full h-full object-cover opacity-60" autoPlay loop muted playsInline />}
                                    ...
                                    <div className={`flex flex-col items-center gap-2 transition-all duration-1000 delay-300 ${segments.length === 0 ? 'opacity-0' : 'opacity-100'}`}>
                                        ...
                                        <span className="text-[9px] font-black text-indigo-500/60 uppercase tracking-widest">NEXT ANCHOR</span>
                                    </div>
                                    <div className="flex-1 w-full max-w-[480px]">
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between px-2">
                                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-indigo-400">{segments.length === 0 ? 'Step 1 Input' : `Step ${segments.length + 1} Anchor`}</span>
                                                <div className="flex items-center gap-2">
                                                    <button
                                                        onClick={() => fileInputRef.current?.click()}
                                                        className="px-2 py-0.5 bg-slate-800 hover:bg-slate-700 rounded text-[9px] font-bold text-slate-400 transition-colors uppercase tracking-widest border border-slate-700"
                                                    >
                                                        Change
                                                    </button>
                                            ...
                                            <div className={`bg-slate-900 rounded-2xl overflow-hidden border-2 border-indigo-500/50 relative group shadow-2xl ${getAspectClass(aspectRatio)}`} style={getContainerStyles(aspectRatio)}>
                                                <img key={currentImage} src={currentImage!} className={`w-full h-full object-cover ${isGenerating ? 'opacity-30 blur-sm' : ''} transition-all duration-700`} alt="Current Anchor" />
                                                {isGenerating && (
                                                    <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 bg-indigo-950/20 backdrop-blur-[2px]">
                                                        <div className="w-16 h-16 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin" />
                                                        <span className="text-white font-black uppercase tracking-[0.3em] text-[10px]">Generating...</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
                                    ...
                                    <div className="space-y-3 relative">
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                                <button onClick={() => setIsSidebarOpen(true)} className="p-1.5 bg-slate-800 hover:bg-indigo-500/20 hover:text-indigo-400 rounded-lg text-slate-400 transition-all flex items-center gap-2 pr-3">
                                            ...
                                            <div className="flex items-center gap-4">
                                                <div className="flex items-center gap-2 px-3 py-1 bg-slate-950/50 border border-slate-800 rounded-lg group hover:border-indigo-500/30">
                                                    <span className="text-[9px] font-black text-slate-500 uppercase">Iterations</span>
                                                    <input type="number" min="1" max="10" value={iterationCount} onChange={(e) => setIterationCount(Math.max(1, parseInt(e.target.value) || 1))} className="w-10 bg-transparent text-center text-xs font-bold text-white focus:outline-none" />
                                                </div>
                                                {prompt && !globalPrompts.includes(prompt) && (
                                                    <button onClick={() => savePrompt(prompt)} className="px-2 py-0.5 bg-indigo-500/10 text-indigo-400 rounded text-[9px] font-bold uppercase tracking-widest border border-indigo-500/20 hover:bg-indigo-500/20">Save</button>
                                                )}
                                                <div className="flex bg-slate-950/80 p-1 rounded-xl border border-slate-800">
                                                    <button onClick={() => setWorkflowType('turbowan')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${workflowType === 'turbowan' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Turbo</button>
                                                    <button onClick={() => setWorkflowType('qwen')} className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${workflowType === 'qwen' ? 'bg-purple-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Qwen</button>
                                                </div>
                                            </div>
                                        </div>
                                        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Describe the action..." className="w-full bg-slate-950/50 border border-slate-800 rounded-2xl p-6 text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all resize-none h-40 font-medium" />
                                    </div>
                                    <button onClick={handleGenerate} disabled={isGenerating || !prompt} className="w-full py-5 bg-gradient-to-r from-indigo-600 to-purple-600 disabled:opacity-50 text-white font-black uppercase tracking-[0.2em] text-sm rounded-2xl shadow-2xl transition-all flex items-center justify-center gap-4 group">
                                        {isGenerating ? (
                                            <><div className="w-5 h-5 border-2 border-white/20 border-t-white rounded-full animate-spin" />Processing...</>
                                        ) : (
                                            <><svg className="w-6 h-6 group-hover:translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M13 7l5 5m0 0l-5 5m5-5H6" /></svg>
                                                {iterationCount > 1 ? 'Generate Sequence' : (segments.length === 0 ? 'Begin' : 'Extend')}</>
                                        )}
                                    </button>
                                </div>

                                <div className="space-y-4 pt-8 border-t border-slate-800/50">
                                    <h3 className="text-[11px] font-black text-slate-500 uppercase tracking-[0.2em]">Storyline ({segments.length})</h3>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {segments.map((seg, i) => (
                                            <div key={i} className="bg-slate-900/50 border border-slate-800 rounded-xl overflow-hidden hover:border-indigo-500/30 group">
                                                <div className={`relative ${getAspectClass(aspectRatio)} cursor-zoom-in`} onClick={() => onPreviewMedia?.(seg.videoUrl)}>
                                                    <video src={seg.videoUrl} className="w-full h-full object-cover" autoPlay loop muted playsInline />
                                                    <div className="absolute top-2 left-2 px-2 py-0.5 bg-black/60 rounded text-[8px] font-black text-white uppercase">Gen {i + 1}</div>
                                                </div>
                                                <div className="p-3"><p className="text-[9px] text-slate-500 italic line-clamp-1 leading-snug">"{seg.prompt}"</p></div>
                                            </div>
                                        ))}
                                        {segments.length === 0 && <div className="col-span-full h-24 border border-dashed border-slate-800 rounded-xl flex items-center justify-center text-[10px] text-slate-600 font-bold uppercase whitespace-nowrap overflow-hidden">Pipeline Empty</div>}
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    {stitchedVideoUrl && (
                        <div className="max-w-2xl mx-auto space-y-6">
                            ...
                            <div className={`bg-black rounded-3xl overflow-hidden shadow-2xl border border-slate-800 ring-1 ring-white/10 ${getAspectClass(aspectRatio)} mx-auto cursor-zoom-in group relative`} style={getContainerStyles(aspectRatio)} onClick={() => onPreviewMedia?.(stitchedVideoUrl)}>
                            ...
                            <div className="flex gap-4">
                                <a href={stitchedVideoUrl} download="final_video.mp4" className="flex-1 py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl transition-all text-center flex items-center justify-center gap-2">
