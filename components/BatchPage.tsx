...
export const BatchPage: React.FC<BatchPageProps> = ({ onPreviewImage, onSendToTurbo, onSendToUpscale }) => {
    ...
    const runBatch = async () => {
        ...
        while (true) {
            ...
            try {
                const result = await ComfyUiService.runQwenSingleEditWorkflow(
                    currentItem.sourceImage,
                    prompt,
                    loraName ? { name: loraName, strength: loraStrength } : undefined
                );
                ...
                // Small delay to allow browser to initiate image fetch and IO to settle
                await new Promise(resolve => setTimeout(resolve, 1000));

                await ComfyUiService.freeMemory();

                // Aggressive cooldown to ensure VRAM is fully released by CUDA driver
                // This prevents the "driver hang" issue requiring a reset
                console.log("[BATCH] Cooling down GPU...");
                        ...
                        <section className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl sticky top-24">
                            ...
                            <div className="space-y-2 pt-2 border-t border-slate-800">
                                <div className="flex items-center justify-between">
                                    ...
                                    {loraName && <span className="text-xs text-emerald-400 font-mono">{loraStrength.toFixed(1)}</span>}
                                </div>
                                <input
                                    type="text"
                                    value={loraName}
                                    onChange={(e) => setLoraName(e.target.value)}
                                    className="w-full bg-slate-950 border border-slate-800 rounded-lg p-2 text-xs mb-2 transition-all focus:ring-1 focus:ring-emerald-500 outline-none text-slate-300 placeholder:text-slate-700"
                                    placeholder="LoRA Filename (optional)"
                                    disabled={isGenerating}
                                />
                                {loraName && (
                                    <input
